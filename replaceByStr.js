...
// String based replace to be completely safe against line number shifts
const m1 = "{/* ══ AI VIDEO / IMAGE INPUT ══ */}";
const i1 = code.indexOf(m1);
if (i1 !== -1) {
  // Find the exact end of the block the user showed (the </div> after {/* Notice */})
  const m2 = "            {/* Notice */}\\n\\n          </div>";
  const m2Alt = "            {/* Notice */}\\r\\n\\r\\n          </div>";
  let i2 = code.indexOf(m2, i1);
  let len = m2.length;
  
  if (i2 === -1) {
    i2 = code.indexOf(m2Alt, i1);
    len = m2Alt.length;
  }

  if (i2 !== -1) {
    code = code.substring(0, i1) + newComponent + code.substring(i2 + len);
